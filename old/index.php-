<?php 
//include ("check_login.php"); 
include("./admin/conn/conn.php");
?>
<!DOCTYPE html>
<html>
<head>
	<meta charset="utf-8"> 
	<title>图书管理系统</title>
	<link rel="stylesheet" href="./admin/common/bootstrap/3.3.7/css/bootstrap.min.css">  
	<script src="./admin/common/jquery/2.1.1/jquery.min.js"></script>
	<script src="./admin/common/bootstrap/3.3.7/js/bootstrap.min.js"></script>
</head>
<body style="margin-left:10%;margin-top:20px;height:50%;width:81%">
<center><span style="font-size:44px;color:black">那坡下43号图书室网站</span></center>
<br><br>

         <form class="form-inline" name="form1" method="post" action="">
          <table bordercolor="#FFFFFF" bgcolor="#9ECFEE">
  <tr>
    <td >
&nbsp;</td>
<span style="font-size:22px;color:black">图书查询（列表）</span>
    <td>&nbsp;&nbsp;请选择查询依据：
      <select name="f" class="form-control" id="f" width="37">
        <option value="<?php echo "b.barcode";?>" selected>条形码</option>       
        <option value="<?php echo "b.bookname";?>" >书名</option>
        <option value="<?php echo "b.author";?>">作者</option>
        <option value="<?php echo "p.pubname";?>">出版社</option>
        <option value="<?php echo "c.name";?>">书架</option>
        <option value="<?php echo "t.typename";?>">类别</option>
                  </select>
<div class="form-group">
    <label class="sr-only" for="name"></label>
    <input name= "key1" type="text" class="form-control" id="key1" style="width:130%;">
  </div>
  <button type="submit" class="btn btn-primary" value="查询" >查询</button>
    </td>
  </tr>
</table>
</form>
<?php
    include("Conn/conn.php");
    $query=mysql_query("select b.*,c.name as bookcasename,p.pubname,t.typename from tb_bookinfo b left join tb_bookcase c on b.bookcase=c.id join tb_publishing p on b.ISBN=p.ISBN join tb_booktype t on b.typeid=t.id");
    $result=mysql_fetch_array($query);
        if($result==false){
    ?> 
          <table width="100%" height="30"  border="0" cellpadding="0" cellspacing="0">
            <tr>
              <td height="36" align="center">暂无图书信息！</td>
            </tr>
          </table>
 <?php
}else{
?>   
<table class="table table-hover">
	<thead>
		<tr  bgcolor="#E6E6FA">
			<th style="width:">条形码</th>
			<th >图书名称</th>
      <th>作者</th>
      <th >出版社</th>
      <th >索书号</th>
      <th >出版时间</th>
      <th >版次</th>
      <th>印刷时间</th>
      <th >印次</th>
      <th >页码数</th>
      <th >价格:元</th>
			<th >类型</th>
      <th>大小</th>
      <th >印张</th>
      <th >字数:千字</th>
		</tr>
	</thead>
	<tbody>

<?php
if($_POST["key1"]==""){
do{
?>
  <!-- <tr>
    <td>&nbsp;<?php echo "ISBN ";echo $result["barcode"];?></td>  
    <td><a href="./admin/book_look.php?id=<?php echo $result["id"]; ?>"><?php echo $result["bookname"];?></a></td>
    <td>&nbsp;<?php echo $result["author"];?></td>
    <td>&nbsp;<?php echo $result["pubname"];?></td>
    <td>&nbsp;<?php echo $result["bookcasename"];echo $result["findnum"];?></td>
    <td>&nbsp;<?php echo $result["pubTime"];?></td>
    <td>&nbsp;<?php echo $result["pubNum"];?></td>    
    <td>&nbsp;<?php echo $result["printTime"];?></td> 
    <td>&nbsp;<?php echo $result["printNum"];?></td>
    <td>&nbsp;<?php echo $result["page"];?></td>      
    <td>&nbsp;<?php echo $result["price"];?></td>  
    <td>&nbsp;<?php echo $result["typename"];?></td>
    <td>&nbsp;<?php echo $result["size"];?></td>      
    <td>&nbsp;<?php echo $result["printPage"];?></td>  
    <td>&nbsp;<?php echo $result["totalWord"];?></td>  
 
    </tr> -->
<?php
  }while($result=mysql_fetch_array($query));
}else{
$f=$_POST["f"];
$key1=$_POST["key1"];
$sql=mysql_query("select b.*,c.name as bookcasename,p.pubname,t.typename from tb_bookinfo b left join tb_bookcase c on b.bookcase=c.id join tb_publishing p on b.ISBN=p.ISBN join tb_booktype t on b.typeid=t.id where $f like '%$key1%'");
$info=mysql_fetch_array($sql);
if($info==true){
do{
?>
  <tr>
    <td>&nbsp;<?php echo "ISBN ";echo $result["barcode"];?></td>  
    <td><a href="./admin/book_look.php?id=<?php echo $result["id"]; ?>"><?php echo $result["bookname"];?></a></td>
    <td>&nbsp;<?php echo $result["author"];?></td>
    <td>&nbsp;<?php echo $result["pubname"];?></td>
    <td>&nbsp;<?php echo $result["bookcasename"];echo $result["findnum"];?></td>
    <td>&nbsp;<?php echo $result["pubTime"];?></td>
    <td>&nbsp;<?php echo $result["pubNum"];?></td>    
    <td>&nbsp;<?php echo $result["printTime"];?></td> 
    <td>&nbsp;<?php echo $result["printNum"];?></td>
    <td>&nbsp;<?php echo $result["page"];?></td>      
    <td>&nbsp;<?php echo $result["price"];?></td>  
    <td>&nbsp;<?php echo $result["typename"];?></td>
    <td>&nbsp;<?php echo $result["size"];?></td>      
    <td>&nbsp;<?php echo $result["printPage"];?></td>  
    <td>&nbsp;<?php echo $result["totalWord"];?></td> 
    </tr>
<?php
}while($info=mysql_fetch_array($sql));
}else{
?>
    <table width="100%" height="30"  border="0" cellpadding="0" cellspacing="0">
       <tr>
         <td height="36" align="center">您检索的图书信息不存在，请重新检索！</td>
       </tr>
    </table>
<?php
}
}
}
?>  
	</tbody>
</table>
</body>



<br/><br/><br/><br/><br/><br/>
<table class="table table-hover">
	<caption style="color:black;font-size:22px;font-style:boder;">图书借阅排行榜</caption>
	<thead>
		<tr bgcolor="#E6E6FA">
			<th>排名</th>
			<th>图书条形码</th>
			<th>图书名称</th>
			<th>图书类型</th>
			<th>书架</th>
			<th>出版社</th>
			<th>作者</th>
			<th>定价(元)</th>
			<th>借阅次数</th>
		</tr>
	</thead>
	<tbody>
		<?php
					$sql=mysql_query("select * from (select bookid,count(bookid) as degree from tb_borrow group by bookid) as borr join (select b.*,c.name as bookcasename,p.pubname,t.typename from tb_bookinfo b left join tb_bookcase c on b.bookcase=c.id join tb_publishing p on b.ISBN=p.ISBN join tb_booktype t on b.typeid=t.id where b.del=0) as book on borr.bookid=book.id order by borr.degree desc limit 10");
					$info=mysql_fetch_array($sql);
					$i=1;
					do{
					?>
		<tr>
				<td ><?php echo $i;?></td>
                <td ><?php echo "ISBN ";echo $info["barcode"];?></td>
                <td ><a href="./admin/book_look.php?id=<?php echo $info["id"]; ?>"><?php echo $info["bookname"];?></a></td>
                <td ><?php echo $info["typename"];?></td>
                <td ><?php echo $info["bookcasename"];?></td>
                <td ><?php echo $info["pubname"];?></td>
                <td ><?php echo $info["author"];?></td>
                <td ><?php echo $info["price"];?></td>
                <td ><?php echo $info["degree"];?></td>
		</tr>
              <?php 
				 $i=$i+1;
				 }while($info=mysql_fetch_array($sql));
				?>
	</tbody>
</table>

</body>
</html>